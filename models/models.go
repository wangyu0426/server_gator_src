...
func init()  {
	...
	strConn := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s",
		svrctx.Get().DbMysqlConfig.DBUser,
		svrctx.Get().DbMysqlConfig.DBPasswd,
		svrctx.Get().DbMysqlConfig.DBHost,
		svrctx.Get().DbMysqlConfig.DBPort,
		svrctx.Get().DbMysqlConfig.DBName)
	logging.Log("connect mysql string: " + strConn)
	db, err := sql.Open("mysql", strConn)

	if err != nil {
		logging.Log(fmt.Sprintf("connect mysql db %s  failed", svrctx.Get().DbMysqlConfig.DBName) + err.Error())
		os.Exit(1)
	}

	logging.Log(fmt.Sprintf("connect mysql db %s OK", svrctx.Get().DbMysqlConfig.DBName))
	defer db.Close()
